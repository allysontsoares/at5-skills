...
# at5-nicemodal — High-Performance Declarative Dialogs and Forms

This autonomous skill documents the **Declarative Dialogs** architecture in the **AT5** ecosystem, eliminating JSX pollution and unnecessary manual NiceModal wrappers. It presents the complete infrastructure code for **`createFormDialog`** and **`createAlertDialog`**, integrated with **React Hook Form**, **Zod**, and the `Modal` compound component from the design system.

---

## 🛠️ 1. The Declarative Philosophy

Instead of controlling manual `isOpen`, `onClose` states in the parent component and polluting the JSX tree with modals, AT5 adopts **NiceModal** wrapped in strict utilities. Any dialog in the system is dispatched declaratively in a sync/async manner:
...
> [!NOTE]
> **Import Aliases**: All code below uses the illustrative aliases `@/components/ui` (or `@/packages/ui`) and `@/lib/utils` to ensure agnostic compatibility. When implementing in a specific project, adapt the import aliases according to the `tsconfig.json` and local directory structure.

---

## 🏗️ 2. Real `createFormDialog` Helper Code
  ...
  return NiceModal.create((props: TProps) => {
    ...
    const handleCloseAttempt = async () => {
      if (isDirtyRef.current) {
        // Import and use createAlertDialog to maintain visual design system consistency
...
## 🏗️ 3. Real `createAlertDialog` Helper Code
...
## 📝 4. Complete Real Example: `UserDialog` Form Dialog
